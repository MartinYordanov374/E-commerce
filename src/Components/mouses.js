...
import {MdShoppingBasket, MdAddShoppingCart, MdHome, MdFavorite, MdAdd} from 'react-icons/md'
...
class mouses extends Component {
    render() {
        ...
        const mouses = [
            ...
            const addProduct=(mouse)=>
            {
                store.dispatch({type: 'addToCart', name: mouse.name, price: mouse.price, image: mouse.image, id: mouse.id})
            }
            const addToWishList=(mouse)=>
            {
                store.dispatch({type: 'addToWishList', name: mouse.name, price: mouse.price, image: mouse.image, id: mouse.id})
            }
        return (
            <div>
                ...
                <h3 className='productName'>{mouse.name}</h3>
                <img className='productImage' src={mouse.image}></img>
                <p className='productPrice'><strong>$ {mouse.price}.99</strong></p>
                <Button  className='addProductButton' variant='warning' onClick={()=>addToWishList(mouse)}> <MdAdd size={20}></MdAdd> <strong>Add to WishList </strong> </Button>
                <Button  className='addProductButton' variant='success' onClick={()=>addProduct(mouse)}> <MdAddShoppingCart size={20}></MdAddShoppingCart> <strong>Add to cart </strong> </Button>
...
export default connect(mapStateToProps)(mouses)
